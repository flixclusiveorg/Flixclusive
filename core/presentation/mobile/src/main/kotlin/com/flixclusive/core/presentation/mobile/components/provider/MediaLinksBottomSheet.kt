...
private typealias SheetViewType = Int

private const val SUBTITLES_PANEL = 1
private const val STREAMS_PANEL = 0

private val SheetViewType.isViewingSubtitles: Boolean get() = this == SUBTITLES_PANEL
private val SheetViewType.isViewingStreams: Boolean get() = this == STREAMS_PANEL
...
fun MediaLinksBottomSheet(
    ...
    var view: SheetViewType by remember { mutableIntStateOf(STREAMS_PANEL) }
    val items = when (view) {
        STREAMS_PANEL -> streams
        SUBTITLES_PANEL -> subtitles
        else -> emptyList()
    }
    ...
    ModalBottomSheet(
        ...
        LazyColumn(
            ...
            item {
                AnimatedVisibility(
                    ...
                    FilterSegmentedButtons(
                        viewType = view,
                        isSubtitleListEmpty = subtitles.isEmpty(),
                        onToggleView = { view = it },
                    )
                }
            }
            ...
            item {
                AnimatedVisibility(
                    visible = items.isEmpty() && state.isError,
...
private fun FilterSegmentedButtons(
    ...
    onToggleView: (SheetViewType) -> Unit,
    ...
    val streamButtonCorner = if (isSubtitleListEmpty) hasCorner else noCorner

    SingleChoiceSegmentedButtonRow(
        modifier = modifier.fillMaxWidth(),
    ) {
        SegmentedButton(
            selected = viewType.isViewingStreams,
            onClick = { onToggleView(STREAMS_PANEL) },
            colors = colors,
            border =
...
@Composable
private fun getDefaultSegmentedButtonColors() =
