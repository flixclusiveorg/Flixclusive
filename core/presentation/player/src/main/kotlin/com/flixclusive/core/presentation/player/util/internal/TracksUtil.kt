...
internal object TracksUtil {
    ...
    /**
     * Appends a numerical suffix to the string if it already exists in the provided set.
     * The suffix is in the format " #X", where X is an incrementing number starting from 1.
     *
     * @param set A set of strings to check for duplicates.
     *
     * @return A unique string with a numerical suffix if needed.
     * */
    fun String.withNumberSuffix(set: HashSet<String>): String {
        var count = 0
        var candidate = this
        while (candidate in set) {
            count++
