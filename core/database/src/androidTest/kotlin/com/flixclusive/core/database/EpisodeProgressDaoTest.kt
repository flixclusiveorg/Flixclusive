...
class EpisodeProgressDaoTest {
    ...
    @Test
    fun shouldInsertAndRetrieveEpisodeProgress() = runTest {
    ...
    @Test
    fun shouldReturnNullForNonexistentEpisodeProgress() = runTest {
        val result = episodeProgressDao.get(0)
        expectThat(result).isNull()
    }

    @Test
    fun shouldGetAllEpisodeProgressForUser() = runTest {
        val user = User(id = 1, name = "testuser", image = 1)
        val tvSeries = DBFilm(
            id = "series123",
            providerId = "provider1",
        ...
        expectThat(allProgress[1].watchData.episodeNumber).isEqualTo(1)
    }

    @Test
    fun shouldReturnEmptyListForUserWithNoEpisodeProgress() = runTest {
        val allProgress = episodeProgressDao.getAllAsFlow(999).first()
        expectThat(allProgress).isEmpty()
    }

    @Test
    fun shouldGetEpisodeProgressForSpecificSeries() = runTest {
        val user = User(id = 1, name = "testuser", image = 1)
        val series1 = DBFilm(
            id = "series1",
            providerId = "provider1",
        ...
        val series1Progress = episodeProgressDao.getAllForSeries("series1", 1).first()
        expectThat(series1Progress).hasSize(1)
        expectThat(series1Progress[0].watchData.filmId).isEqualTo("series1")
    }

    @Test
    fun shouldUpdateEpisodeProgress() = runTest {
        val user = User(id = 1, name = "testuser", image = 1)
        val tvSeries = DBFilm(
            id = "series123",
            providerId = "provider1",
    ...
    @Test
    fun shouldDeleteEpisodeProgress() = runTest {
        val user = User(id = 1, name = "testuser", image = 1)
        val tvSeries = DBFilm(
            id = "series123",
            providerId = "provider1",
    ...
    @Test
    fun shouldHandleMultipleEpisodesInSameSeason() = runTest {
        val user = User(id = 1, name = "testuser", image = 1)
        val tvSeries = DBFilm(
            id = "series123",
            providerId = "provider1",
        ...
        // Insert progress for multiple episodes in season 1
        ...
        val seriesProgress = episodeProgressDao.getAllForSeries("series123", 1).first()
        expectThat(seriesProgress).hasSize(5)
        expectThat(seriesProgress.map { it.watchData.episodeNumber }).isEqualTo(listOf(1, 2, 3, 4, 5))
    }

    @Test
    fun shouldHandleMultipleSeasonsForSameSeries() = runTest {
        val user = User(id = 1, name = "testuser", image = 1)
        val tvSeries = DBFilm(
            id = "series123",
            providerId = "provider1",
        ...
        val allProgress = episodeProgressDao.getAllForSeries("series123", 1).first()
        expectThat(allProgress).hasSize(2)
        expectThat(allProgress[0].watchData.seasonNumber).isEqualTo(1)
        expectThat(allProgress[1].watchData.seasonNumber).isEqualTo(2)
    }

    @Test
    fun shouldGetRandomEpisodeProgress() = runTest {
        val user = User(id = 1, name = "testuser", image = 1)
        val tvSeries = DBFilm(
            id = "series123",
            providerId = "provider1",
