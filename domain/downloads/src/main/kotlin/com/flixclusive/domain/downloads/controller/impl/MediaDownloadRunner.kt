 ...
 *
 * Split out of [MediaDownloadControllerImpl] so the scheduler -- concurrency slots, the job map and
 * the network gate -- stays separable from the transfer engine underneath it. Nothing here touches
 * scheduling state; the only entry point is [run].
 */
@Singleton
internal class MediaDownloadRunner @Inject constructor(
    ...
    /** Also used by the controller's delete/stop paths, which tear the same directory down. */
    suspend fun resolveDirectory(item: DownloadItem): UniFile? =
    ...
    private suspend fun currentLinkSortDirection(): DownloadLinkSortDirection =
        dataStoreManager
            .getUserPrefs<DataPreferences>(UserPreferences.DATA_PREFS_KEY)
