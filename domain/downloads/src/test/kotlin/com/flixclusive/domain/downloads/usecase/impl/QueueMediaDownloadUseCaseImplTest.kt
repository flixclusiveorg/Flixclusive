...
class QueueMediaDownloadUseCaseImplTest {
    ...
    fun setup() {
        ...
        useCase = QueueMediaDownloadUseCaseImpl(mediaDownloadRepository, mediaDownloadController)
    }
    ...
    fun `invoke should queue a QUEUED item with no source url for a movie`() =
        runTest {
            ...
            expectThat(result).isEqualTo(itemSlot.captured.id)
            // Queueing starts the download; that is the contract, not the caller's job.
            verify { mediaDownloadController.start(itemSlot.captured.id) }
