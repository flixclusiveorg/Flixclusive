...
@Singleton
class HomeItemsProviderUseCase
    @Inject
    constructor(
        ...
        suspend fun getFocusedFilm(film: Film) {
            _state.update {
                val focusedFilm = getFilmMetadataUseCase(partiallyDetailedFilm = film).data
                ...
                while (headerItem == null && catalogAttempts < HOME_MAX_PAGE) {
                    ...
                    if (headerItem != null && !traversedFilms.contains(headerItem.identifier)) {
                        ...
                        if (!headerItem.isNotPopular) {
                            val response = getFilmMetadataUseCase(partiallyDetailedFilm = headerItem)
