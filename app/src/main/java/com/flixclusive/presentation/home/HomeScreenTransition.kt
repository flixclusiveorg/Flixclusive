...
object HomeScreenTransition : DestinationStyleAnimated {
    override fun AnimatedContentTransitionScope<NavBackStackEntry>.enterTransition(): EnterTransition? {
        return when(initialState.appDestination()) {
            RecentlyWatchedScreenDestination, SearchScreenDestination, WatchlistScreenDestination -> slideInHorizontally { -1000 } + fadeIn()
            else -> null
        }
    }
    ...
    override fun AnimatedContentTransitionScope<NavBackStackEntry>.exitTransition(): ExitTransition? {
        return when(targetState.appDestination()) {
            RecentlyWatchedScreenDestination, SearchScreenDestination, WatchlistScreenDestination -> fadeOut() + slideOutHorizontally { -1000 }
            else -> null
        }
    }
}