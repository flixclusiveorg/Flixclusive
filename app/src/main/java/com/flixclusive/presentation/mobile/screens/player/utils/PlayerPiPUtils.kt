...
import com.flixclusive.presentation.common.player.PlayerEvents
import com.flixclusive.presentation.mobile.screens.player.ACTION_PIP_CONTROL
import com.flixclusive.presentation.mobile.screens.player.PLAYER_PIP_EVENT

object PlayerPiPUtils {
    ...
    fun Activity.updatePiPParams(
        ...
        val params = with(PictureInPictureParams.Builder()) {
            ...
            val (playPauseIcon, eventCode) = when {
                ...
                else -> R.drawable.play to PlayerEvents.PLAY
            }

            val actions = arrayListOf(
                getRemoteAction(
                    icon = backwardSeekIcon,
                    label = R.string.backward_button_content_description,
                    event = PlayerEvents.FORWARD
                ),
                getRemoteAction(
                    icon = playPauseIcon,
                    label = R.string.play_pause_button_content_description,
                    event = eventCode
                ),
                getRemoteAction(
                    icon = forwardSeekIcon,
                    label = R.string.forward_button_content_description,
                    event = PlayerEvents.FORWARD
                ).also {
