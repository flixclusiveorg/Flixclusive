...
@Composable
fun SlidingAudioAndDisplaySheet(
    ...
    val audiosListState = rememberTvLazyListState()
    ...
    val shouldFocusOnAudios = remember { mutableStateOf(false) }

    LaunchedEffect(Unit) {
        qualitiesListState.scrollToItem(selectedQuality)
        subtitlesListState.scrollToItem(selectedSubtitle)
        audiosListState.scrollToItem(selectedAudio)
    }
                ...
                itemsIndexed(audios) { i, audio ->
                    SlidingSheetItem(
                        modifier = Modifier
                            ...
                            .ifElse(
                                condition = sideSheetFocusPriority == BottomControlsButtonType.Audio && i == selectedAudio,
                                ifTrueModifier = Modifier.focusOnInitialVisibility(
                                    shouldFocusOnAudios
                                )
                            ),
