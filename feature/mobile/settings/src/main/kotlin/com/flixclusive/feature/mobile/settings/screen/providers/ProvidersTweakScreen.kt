...
internal class ProvidersTweakScreen(
    ...
    override fun getDescription(): String
        = stringResource(LocaleR.string.providers_settings_content_desc)
    ...
    private fun getGeneralTweaks(
        ...
        return TweakGroup(
            ...
            tweaks = persistentListOf(
                TweakUI.SwitchTweak(
                    value = remember { mutableStateOf(providerPreferences.autoUpdate) },
                    title = stringResource(LocaleR.string.auto_update_providers),
                    onTweaked = {
                        onUpdatePreferences { oldValue ->
                            oldValue.copy(autoUpdate = it)
                        }
                    }
                ),
                TweakUI.SwitchTweak(
                    value = remember { mutableStateOf(providerPreferences.warnOnInstall) },
                    ...
                    onTweaked = {
                        onUpdatePreferences { oldValue ->
                            oldValue.copy(warnOnInstall = it)
                        }
                    }
                )
            )
    ...
    private fun getDataTweaks(
        ...
        val formatWarningMessage = fun (action: String): String {
            return context.getString(LocaleR.string.action_warning_format_message, action)
        }

        val formatWarningCountDescription = fun (items: Int): String {
            return context.getString(LocaleR.string.warn_delete_items_format, items)
        }
