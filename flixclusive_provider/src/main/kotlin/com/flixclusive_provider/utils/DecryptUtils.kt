...
object DecryptUtils {
    ...
    fun decryptAes(encryptedData: String, key: String): String {
        return try {
            val cipherData = Base64.getDecoder().decode(encryptedData)
            val saltData: ByteArray = Arrays.copyOfRange(cipherData, 8, 16)

            val md5 = MessageDigest.getInstance("MD5")
            val keyAndIV = generateKeyAndIV(saltData, key.toByteArray(), md5) ?: throw Error("Null KeyAndIV")
    ...
    fun extractEmbedDecryptionDetails(encryptedString: String, stops: List<List<Int>>): Pair<String, String> {
        var decryptedKey = ""
        var offset = 0
