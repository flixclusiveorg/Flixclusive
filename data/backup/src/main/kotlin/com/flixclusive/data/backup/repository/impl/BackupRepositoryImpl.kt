...
internal class BackupRepositoryImpl @Inject constructor(
        ...
        return withContext(appDispatchers.io) {
            ...
            val backup = Backup(
                ...
                searchHistory = if (options.includeSearchHistory) {
                    searchHistoryBackupCreator().getOrThrow()
                } else {
                    emptyList()
                },
            ...
            val result = BackupResult(
                missingLibraryLists = if (options.includeLibrary) {
                    libraryListBackupValidator(
                ...
                missingPreferences = if (options.includePreferences) {
                    preferenceBackupValidator(
                ...
                missingWatchProgress = if (options.includeWatchProgress) {
                ...
                missingSearchHistory = if (options.includeSearchHistory) {
                    searchHistoryBackupValidator(
                ...
                missingProviders = if (options.includeProviders) {
                    providerBackupValidator(
    ...
    override suspend fun restore(uri: Uri, options: BackupOptions): BackupResult {
        return withContext(appDispatchers.io) {
            ...
            BackupResult(
                missingLibraryLists = if (includeLibrary) {
                    libraryListBackupValidator(
                        backup = backup.libraryLists,
                ...
                missingProviders = if (includeProviders) {
                    providerBackupValidator(
                        backup = backup.providers,
                        mode = BackupValidationMode.RESTORE,
                    ).getOrThrow()
                ...
                missingProviderRepositories = if (includeRepositories) {
                    repositoryBackupValidator(
                        backup = backup.repositories,
                        mode = BackupValidationMode.RESTORE,
                    ).getOrThrow()
                ...
                missingPreferences = if (includePreferences) {
                    preferenceBackupValidator(
                        backup = backup.preferences,
                        mode = BackupValidationMode.RESTORE,
                    ).getOrThrow()
                ...
                missingSearchHistory = if (includeSearchHistory) {
                    searchHistoryBackupValidator(
                        backup = backup.searchHistory,
                        mode = BackupValidationMode.RESTORE,
                    ).getOrThrow()
